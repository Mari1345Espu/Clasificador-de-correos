...
try:
    ...
    print("Error: The file 'spam_dataset (1).csv' was not found.")
    exit()

# Seleccionar las variables predictoras (features) y la variable objetivo (label)
...
for i in range(num_executions):
    ...
    model_filename = f'logistic_regression_model_run_{i+1}.joblib'
...
plt.legend()
plt.grid(True)
plt.savefig('metrics_history_plot.png')
plt.close()

print("Gráfico del historial de métricas generado y guardado como 'metrics_history_plot.png'.")
...
plt.ylim(-0.05, 1.05)
plt.legend()
plt.savefig('final_sigmoid_predictions_plot.png')
plt.close()

print("Gráfico de la línea de regresión final generado y guardado como 'final_sigmoid_predictions_plot.png'.")